...
if (staticForm) {
  ...
  staticForm.addEventListener('submit', (event) => {
    ...
    try {
      ...
      stringChecker(output[2], 'Floor');
      checkValidFloor(output[1],output[2]);
      stringChecker(output[3], 'Description');
      stringChecker(output[4], 'Noise Rating');
      checkRating(output[4], 'Noise Rating');
      stringChecker(output[5], 'Location Rating');
      checkRating(output[5], 'Location Rating');
      stringChecker(output[6], 'NYC Rating');
      checkRating(output[6], 'NYC Rating');
      stringChecker(output[7], 'Student Cap');
      checkStudentCapacity(output[7], 'Student Cap');
      submit()
    } catch (e) {
